...
const MyBookList = ({ navigation }) => {
  ...
  const [refreshing, setRefreshing] = useState(false); // 2. New state for pull-to-refresh
  const [addingToCartId, setAddingToCartId] = useState(null);

  // 3. Extracted fetch logic into a reusable function
  const fetchCatalog = async (isRefresh = false) => {
    try {
      if (isRefresh) {
        setRefreshing(true);
      } else {
        ...
        <View style={styles.detailsContainer}>
          ...
          <View style={styles.actionRow}>
            <TouchableOpacity
              style={[
                styles.addToCartBtn,
                item.stock === 0 && styles.outOfStockBtn
              ]}
              disabled={item.stock === 0 || addingToCartId === item._id}
              onPress={() => handleAddToCart(item)}
            >
              {/* Added a small loading state for the individual button while adding to cart */}
        ...
        <FlatList
          ...
          refreshControl={
            <RefreshControl
              ...
              colors={['#4F46E5']} // Android loader color
              tintColor="#4F46E5"  // iOS loader color
            />
          }
        />
      )}
...
const styles = StyleSheet.create({
  ...
  header: { paddingHorizontal: 20, paddingTop: 10, paddingBottom: 16, backgroundColor: '#ffffff', borderBottomWidth: 1, borderBottomColor: '#f1f5f9' },
  headerTopRow: { marginBottom: 10 },
  backButtonContainer: { alignSelf: 'flex-start', paddingVertical: 5, paddingRight: 15 },
  headerTitle: { fontSize: 28, fontWeight: 'bold', color: '#0f172a' },
  headerSubtitle: { fontSize: 14, color: '#64748b', marginTop: 4 },
  listContainer: { padding: 16, flexGrow: 1 },
  card: { flexDirection: 'row', backgroundColor: '#ffffff', borderRadius: 16, marginBottom: 16, padding: 12, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 8, elevation: 3 },
  imageContainer: { width: 100, height: 145, borderRadius: 10, overflow: 'hidden', backgroundColor: '#e2e8f0', position: 'relative' },
  coverImage: { width: '100%', height: '100%' },
  formatBadge: { position: 'absolute', bottom: 0, left: 0, right: 0, backgroundColor: 'rgba(79, 70, 229, 0.9)', paddingVertical: 4, alignItems: 'center' },
  formatText: { color: '#fff', fontSize: 10, fontWeight: 'bold', letterSpacing: 1 },
  detailsContainer: { flex: 1, marginLeft: 16, justifyContent: 'space-between' },
  categoryText: { fontSize: 12, color: '#F27A21', fontWeight: '700', textTransform: 'uppercase', marginBottom: 4 },
  title: { fontSize: 16, fontWeight: 'bold', color: '#F27A21', marginBottom: 4 },
  ...
  discountPrice: { fontSize: 18, fontWeight: 'bold', color: '#0f172a' },
  originalPrice: { fontSize: 13, color: '#94a3b8', textDecorationLine: 'line-through' },
  actionRow: { marginTop: 'auto' },
  addToCartBtn: { backgroundColor: '#F27A21', paddingVertical: 10, borderRadius: 8, alignItems: 'center', justifyContent: 'center' },
  outOfStockBtn: { backgroundColor: '#cbd5e1' },
  addToCartText: { color: '#ffffff', fontSize: 14, fontWeight: '700' },
